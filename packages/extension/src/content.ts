...
import { buildValidationReport, isUsableRule } from '@formready/engine';
import type { ConfirmedRequirements, DocumentKind, ImageFormat, Rule } from '@formready/engine';
...
async function runPipeline(
  ...
  try {
    ...
    if (needsPdf) {
      const pdfBlob = await convertToPdfViaBackground(asBlob(blob), metadata.format);
      attachFileToInput(input, pdfBlob, `${baseName}.pdf`, 'application/pdf');
      panel.innerHTML = `
      ...
      attachFileToInput(input, asBlob(blob), `${baseName}.${ext}`, `image/${metadata.format}`);
      panel.innerHTML = `
