 ...
 *
 * The engine moves document bytes around but never decodes them — that is the
 * platform's job. Declaring the shape structurally rather than importing `Blob`
 * is what keeps this package free of browser types: a DOM `Blob` satisfies it
 * as-is, and an Android adapter supplies its own equivalent.
 */
...
/** Cancellation, structurally. A DOM `AbortSignal` satisfies this. */
