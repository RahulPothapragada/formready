...
import { fitWithinAspect } from '../geometry/fit';
import type {
  BinarySource,
  CancellationSignal,
  CandidateMetadata,
...
export async function searchCandidates(context: SearchContext): Promise<SearchOutcome> {
  ...
  // Date.now() rather than performance.now(): the budget is in
  // milliseconds and the engine must not assume a browser clock exists.
