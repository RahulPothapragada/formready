...
import { createJob, jobReducer } from '@formready/engine';
import type { Candidate, Job, Rule, SourceDocument } from '@formready/engine';
...
describe('round trip', () => {
  ...
  it('keeps the original blob intact through storage', async () => {
    ...
    expect(await (restored.source!.blob as Blob).text()).toBe('original');
  });
