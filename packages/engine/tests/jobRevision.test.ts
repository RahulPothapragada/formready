...
import { canExport, createJob, jobReducer, type JobAction } from '@formready/engine';
import type { Candidate, Job, Rule, SourceDocument, ValidationReport } from '@formready/engine';
...
describe('original preservation', () => {
  it('never replaces the source blob during preparation or export', async () => {
    ...
    expect(await (exported.source!.blob as Blob).text()).toBe('original');
  });
