...
import { searchCandidates, type SearchBudget } from '@formready/engine';
import { decode, encode, orientedSize, render } from '@formready/browser';
...
async function prepare(request: PrepareRequest): Promise<void> {
  ...
  try {
    ...
    post({
      ...
      runId,
      jobRevision,
      blob: outcome.result.blob as Blob,
